...
Rules enforced (HARD — build fails on violation):
  R1  No deprecated zwart-wit Unicode glyphs (✓ ✗ ★ → ▶) in any icon field
  ...
  R9  Pill present (decks with 3+ content slides)
  R10 Example/Voorbeeld card present (decks with 3+ content slides)
...
# Deprecated zwart-wit Unicode glyphs — forbidden in any icon field
DEPRECATED_GLYPHS = {"✓", "✗", "★", "→", "▶", "●", "○", "■", "□"}
...
def _check_r1_deprecated_glyphs(slides: list) -> list:
    """R1: No deprecated zwart-wit Unicode glyphs (✓ ✗ ★ → ▶) in icon fields."""
    issues = []
...
def _check_r9_pill_present(slides: list) -> list:
    """R9: at least 1 pill badge anywhere in decks with 3+ content slides."""
    issues = []
    if len(slides) < 3:
        return issues
    for slide in slides:
    ...
    issues.append(ValidationIssue(
        rule="R9",
        slide_index=None,
        message="no pill badge anywhere in deck (3+ content slides need ≥1 pill)",
...
def _check_r10_example_present(slides: list) -> list:
    """R10: at least 1 Example/Voorbeeld card anywhere in decks with 3+ content slides."""
    issues = []
    if len(slides) < 3:
        return issues
    for slide in slides:
    ...
    issues.append(ValidationIssue(
        rule="R10",
        slide_index=None,
        message="no Example/Voorbeeld card anywhere in deck (3+ content slides need ≥1)",
